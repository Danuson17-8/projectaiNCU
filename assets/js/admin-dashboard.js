...
import { statusLabel, statusBadgeClass } from './statusUtils.js';
...
function niceScale(max, tickCount = 4) {
  ...
  const step = [1, 2, 5, 10].map((m) => Math.max(1, m * mag)).find((s) => s >= raw);
...
function ticketsInRange(days) {
  ...
  return start ? allTickets.filter((t) => new Date(t.created_at) >= start) : allTickets;
}
...
function renderKpis(tickets) {
  ...
  document.getElementById('kpi-pending-share').textContent = `${pct(counts.pending, total)}% ของทั้งหมด`;
  document.getElementById('kpi-progress-share').textContent = `${pct(counts.in_progress, total)}% ของทั้งหมด`;
  document.getElementById('kpi-resolved-share').textContent = `${pct(counts.resolved, total)}% ของทั้งหมด`;
  ...
  const pctText = prev ? ` (${diff > 0 ? '+' : ''}${Math.round((diff / prev) * 100)}%)` : '';
...
function buildSeries(tickets) {
  ...
  if (!start) {
    const first = tickets.reduce((min, t) => Math.min(min, new Date(t.created_at).getTime()), today.getTime());
  ...
  for (let i = 0; i < spanDays; i += bucketDays) {
    ...
    const to = new Date(from.getTime() + (Math.min(bucketDays, spanDays - i) - 1) * DAY_MS);
...
function renderTrend(tickets) {
  ...
  document.getElementById('trend-sub').textContent = bucketDays === 1 ? 'จำนวนต่อวัน' : 'จำนวนต่อสัปดาห์';
  ...
  for (let v = 0; v <= max; v += step) {
    svg.appendChild(svgEl('line', {
      x1: m.left, x2: m.left + w, y1: y(v), y2: y(v),
      class: v === 0 ? 'dash-axis' : 'dash-grid',
    }));
    const label = svgEl('text', { x: m.left - 8, y: y(v) + 4, 'text-anchor': 'end', class: 'dash-tick' });
  ...
  for (let k = 0; k < xTickCount; k++) {
    const i = xTickCount === 1 ? 0 : Math.round((k / (xTickCount - 1)) * (points.length - 1));
    const anchor = k === 0 ? 'start' : k === xTickCount - 1 ? 'end' : 'middle';
    const label = svgEl('text', { x: x(i), y: height - 8, 'text-anchor': anchor, class: 'dash-tick' });
    label.textContent = shortDate(points[i].from);
    svg.appendChild(label);
  }

  // area wash + line
  const linePath = points.map((p, i) => `${i ? 'L' : 'M'}${x(i)},${y(p.count)}`).join(' ');
  ...
  svg.appendChild(svgEl('circle', { cx: x(last), cy: y(points[last].count), r: 4, class: 'dash-marker' }));
  const endLabel = svgEl('text', { x: x(last) + 10, y: y(points[last].count) + 4, class: 'dash-endlabel' });
  endLabel.textContent = fmt(points[last].count);
  svg.appendChild(endLabel);

  // hover layer: crosshair + dot + tooltip
  const cross = svgEl('line', { y1: m.top, y2: m.top + h, class: 'dash-crosshair', visibility: 'hidden' });
  const hoverDot = svgEl('circle', { r: 5, class: 'dash-marker', visibility: 'hidden' });
  ...
  const onMove = (e) => {
    ...
    const i = points.length === 1 ? 0 : Math.max(0, Math.min(last, Math.round(((px - m.left) / w) * last)));
    const p = points[i];
    ...
    const dateText = bucketDays === 1 ? shortDate(p.from) : `${shortDate(p.from)} – ${shortDate(p.to)}`;
    showTooltip(`<span class="dash-tt-muted">${dateText}</span><strong>${fmt(p.count)} เรื่อง</strong>`, e.clientX, e.clientY);
  };
  const onLeave = () => {
...
function renderSystemBars(tickets) {
  ...
  const bySystem = new Map(allSystems.map((s) => [s.id, { name: s.name, pending: 0, in_progress: 0, resolved: 0, total: 0 }]));
  tickets.forEach((t) => {
    if (!bySystem.has(t.system_id)) {
      bySystem.set(t.system_id, { name: t.systems?.name || '(ไม่ทราบระบบ)', pending: 0, in_progress: 0, resolved: 0, total: 0 });
    }
  ...
  rows.forEach((row) => {
    ...
    STATUSES.forEach((status) => {
      ...
      const html = `<span class="dash-tt-muted">${escapeHtml(row.name)}</span>`
      ...
      seg.addEventListener('pointermove', (e) => showTooltip(html, e.clientX, e.clientY));
      seg.addEventListener('pointerdown', (e) => showTooltip(html, e.clientX, e.clientY));
...
function renderAging() {
  ...
  if (!open.length) {
    list.innerHTML = '<li class="dash-empty">ไม่มีเรื่องค้าง แก้ไขครบทุกเรื่องแล้ว</li>';
    return;
  }
...
async function loadData() {
  const [ticketsRes, systemsRes] = await Promise.all([
    supabase.from('tickets').select('id, message, status, created_at, system_id, systems(name)'),
    supabase.from('systems').select('id, name').order('created_at', { ascending: true }),
  ]);

  if (ticketsRes.error || systemsRes.error) {
    document.getElementById('dash-updated').textContent = 'โหลดข้อมูลไม่สำเร็จ กรุณาลองใหม่';
